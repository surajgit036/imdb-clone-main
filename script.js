...
function renderList(actionForButton){
	...
	for(let i = 0; i<currentMovieStack.length; i++){
		...
		<button id="${currentMovieStack[i].id}" onclick="getMovieInDetail(this)" class="movie-btn">
		<iconify-icon icon="pajamas:details-block" style="color: skyblue;" width="24"></iconify-icon>
		 Movie Details
		 </button>

		<button onclick="${actionForButton}(this)" class="add-to-favourite-button movie-btn" data-id="${currentMovieStack[i].id}" >
		<iconify-icon icon="mdi:movie-favorite-outline" style="color: skyblue;" width=24></iconify-icon>
...
// render the details of selected movie
